/**
 * 移植自 `deepseek-ai/deepseek-harness` `apps/desktop/src/profile-packages.ts`（MIT）。
 *
 * [ported] 除头注释与导入路径外逐字移植（`./runtime-tree.ts` → `./runtimeTree`），
 * 每一处校验分支与错误文案都保持上游语义：
 *  - 状态文件 schemaVersion/runtimeId/links 校验、重复 link 拒绝；
 *  - 只移除「自己记录过」的链接，遇到非链接或不匹配目标的目录硬报错
 *    （refusing to replace unowned package）；
 *  - 依赖图校验：共享包必须是指向运行时树的链接、私有包不得越出 profile、
 *    宿主包必须以 peer 依赖声明且版本满足范围。
 *
 * 共享包清单由 desktop-runtime.json 提供（runtime.sharedPackages），因此
 * `dsh-desktop-bridge` 与 `@deepseek-ai/dsh`、`dsh-desktop-host` 一样被逐一校验，
 * 无需本文件特判——它是本壳 `sharedPackages` 列表中的一员。
 */
...
import { desktopRuntimeId, runtimePath, type DesktopRuntimeDescriptor } from './runtimeTree.ts'
