 ...
 *
 * 与旧实现的根本差异：官方桌面架构里没有 harness 的 web 端口可转发（Host 走字节管道，
 * 不存在监听 socket），因此本模块不再「反向代理到 127.0.0.1:<port>」，而是把 HTTP 请求
 * 直接喂给 Host 的管道 fetch（与 appProtocol 同一个 forward）——局域网设备拿到的页面、
 * `/api`、`/.dsh/remote-stream` 与桌面窗口完全同源同实现。
 *
 * 门禁（两道，缺一不可）：
