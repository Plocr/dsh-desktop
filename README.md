...
- **局域网访问**（托盘 → 设置 → 局域网访问）：开启后 web server 绑定到本机局域网 IP（官方禁止 `0.0.0.0`，故绑定具体网卡 IP 并放行 `--trusted-host`），同网段设备浏览器打开「局域网地址」即可访问；点击「局域网地址」可复制到剪贴板。⚠️ 开启后**同网段任何人**都能打开并驱动 agent（可读文件/执行命令），建议仅在可信网络使用，用毕关闭。
