...
| D40 | **上游 dsh 更新走 GitHub Actions 无人值守**（`scripts/upstream-dsh.mjs` + `.github/workflows/upstream-dsh.yml`）：定时（每天 04:00 CST）拉 npm registry 的**全部版本**取最高 → 高于当前 pin 就同步 `package.json.dshRuntime.dsh` + `packages/host` 里跟随 dsh 的依赖、壳版本 patch+1 → 重建运行时树 + `verify:runtime` + typecheck + 单测 + e2e（真实 dsh + 官方 Web Host + bridge）→ 全绿后写 `docs/release-notes/v<版本>.md`（正文写明「跟随官方最新 harness 同步：旧 → 新」）、把 bump 落到默认分支并打 `v<壳版本>` tag、显式 dispatch `build-release.yml` 三平台出包 + 发 Release（用户端 electron-updater 自动更新）；手动触发取消勾选 `publish` 则退回"只推分支 + 开 PR" | 没有服务器也要能吃到官方更新；但"壳 + dsh + Node + pnpm 是一个签名更新单元"（D29）意味着必须有人同步 pin 并出包。四个实测事实决定了实现：① **npm `latest` dist-tag 会低于最高版本**（latest=0.1.5-rc.2，最高=0.1.6-alpha.2），所以按版本列表取最大值；② electron-updater 只认更高的**壳**版本，所以每次同步必须同时 patch+1；③ 用 GITHUB_TOKEN 推 tag **不会**触发 push 事件，所以出包靠显式 dispatch；④ 只推分支不推默认分支会让 pin 永远不动、天天重复发同一版，所以自动出包路径把 bump 落到默认分支（tag 已存在则 `git tag` 直接失败，天然挡重复发布）。更新日志跟着代码提交而不是用 `--generate-notes`，用户在应用内看到的版本说明与仓库一致 |
| D41 | **桥接监听/连接句柄一律 `unref()`；壳的退出流程只留一个出口**：`packages/bridge` 自持 HTTP 监听句柄（`noServer` 模式）+ `unref()`，每个已接受连接也 `unref()`（dispose 仍显式 close）；壳在退出时**先撤托盘与窗口**（用户可见反馈 ~4ms）、`before-quit` 始终 `preventDefault()`、6s 兜底先 `killNow()` 再 `app.exit(0)`；托盘补 `click`/`double-click` → 显示工作台；托盘菜单加**内容指纹**，没变就不 `setContextMenu` | 性能审查实测（§9）：官方 `createProcessShutdown` 正常路径只设 `process.exitCode` 并等事件循环 drain，因此**任何 ref 的监听/连接句柄都会让 Host 永不退出**（修复前：shutdown 后 60s 仍在、端口仍监听 → 壳只能 5–8s 兜底 SIGKILL → "退出两次/等很久"，`restart()` 还会 7s 超时留旧进程）。`unref` 后同一场景 ~2s 干净退出，实机点退出 **105ms** 完成。菜单指纹解决"Windows 上菜单开着被替换导致点击丢失"（点了没反应）；托盘 click 解决"左键点图标没反应"。启动侧无壳可优化项（壳 0.25s，官方组合树 boot 2.4s） |
| D42 | **桥接按「宿主 API 代」接线，不猜版本号**：`jobsApiGeneration(jobs)`（纯函数，单测锁住）只认新代独有的 `jobs.events.subscribe` —— 新代（dsh ≥ 0.1.7）用 `jobs.events.subscribe({owners:'all'}, …)`（`registered/progress/stopping/removed` → `jobs.changed`，`settled` → `job.done`，`output` 忽略），旧代（≤ 0.1.6）仍走 `onJobsChanged` / `onJobDone`；同一判代决定 `jobs.list()` 的 caller 形状（旧代 live Agent / 新代 `SessionId`）。诊断 `jobs.present` 改在**接线成功之后**发，一个监听器都没接上就只留 `jobs.watch.failed` | dsh 0.1.7-alpha.1 删掉了两代监听器、改成事件流，并把 caller 从 live Agent 换成 `SessionId`，**没有并存期**：`onJobDone is not a function` 让 2026-09-22 的自动巡检在 e2e 挂掉（保护性失败，没发版）。两代都留是因为"壳 + dsh + Node + pnpm 是一个签名更新单元"（D29）——本地开发机与 CI 的运行时树未必同代，而**传错代不抛错**，只会让 caller 与 owner 对不上、可见集静默退化成"只剩无主任务"（通知时有时无），所以接线路径必须自己判代。`settled` 不论 `awaited` 与否都推 `job.done`：`awaited` 是给模型侧 reporter 去重的，壳的通知面向用户，与旧代逐条通知保持一致。`jobs.present` 后置是因为壳按**最新一条**诊断判断可用性（D33），否则"服务在、但没接上"会显示成正常 |
| D43 | **自动更新链路的四个实测事实（一次完整体检的结论，均已落地）**：① Windows 的 `<exe>.blockmap` 必须与安装包一起上传；② macOS 的自动更新**只认 zip**，未签名构建干脆不发 zip，壳按清单诚实提示手动下载；③ 签名分支把 zip 改成「公证+钉票 .app 之后重打」；④ 安装改 `quitAndInstall(true, true)`（静默 + 强制重启），并在失败时复位 `quitForUpdateInstall` | ① 0.8.3 / 0.8.4 两次 CI 出包都漏了 blockmap（手动出的 0.8.0 / 0.8.1 反而有），而 `AppUpdater.differentialDownloadInstaller` 的 catch 会 `return true` 退回整包下载——**只是慢，不是坏**，所以最容易长期没人发现（每次更新多下 ~160 MB）；② `MacUpdater` 的 `findFile(files,'zip',['pkg','dmg'])` 只匹配 zip，缺 zip 时 `downloadUpdate()` 必抛 `ERR_UPDATER_ZIP_FILE_NOT_FOUND`，而 `update-available` 早就发过了：用户此前看到的是「弹了通知然后什么都没有」（自动检查）或一句「检查更新失败」（手动检查）；③ 钉票会改 .app 字节，打包期的 zip 与 `<zip>.blockmap` 因此作废，必须重打并删块图，否则差分下载拿到错的基准、最后以 sha512 校验失败收场（CI 上传步骤显式拒绝残留的 zip blockmap 以防回归）；④ `autoInstallOnAppQuit=false` 时安装靠用户点按钮，`quitAndInstall()` 默认会弹 NSIS 向导且不强制重启，与对话框承诺的「安装完成后自动重启」不符。**已实测**：v0.8.4 的 setup.exe（168,505,418 字节）sha512 与 `latest.yml` 完全一致、`app-update.yml` 的 provider/owner/repo 与 `releases/latest` 解析正确、运行时清单 `release.version` = 应用版本（D29 的「一个签名更新单元」没被 CI 缓存绕过）。macOS 侧的 zip 流程**无法在本机验证**（需 Apple Developer ID 与一次 macOS runner 运行），因此壳侧保留了不依赖签名的诚实降级 |
...
| D33 | **诊断通道 `bridge.diag`**：插件把 `ws.listening`/`jobs.present|absent`/`jobs.watch.failed`/`auth.rejected`/`loader.rejected`/`protocol.mismatch` 既打 stdout 也推给已鉴权连接，最新一条随 `authed` 给壳；壳落日志并显示「桥接：已连接 · jobs present」 | 桥接失效的表现是"通知不响"，此前只能翻与 harness 混流的 stdout；诊断通道让"连上了吗 / jobs 在不在"变成一眼可见的托盘状态 |
...
- 事件推送（最小字段，绝不序列化 live 对象）：
  - `jobs.changed`：`{jobs:[{id,kind,label,status,owner}]}` —— 任务可见集变化（逐会话聚合，无主任务按 id 去重）；
  - `job.done`：`{job:{…}}` —— 单个任务结算（视图含 label/kind，通知可读）；
  - 上面两条的**宿主 API 按代接线**（`jobsApiGeneration`，见 D42）：新代（≥0.1.7）`jobs.events.subscribe({owners:'all'}, …)`，
    `registered/progress/stopping/removed` → `jobs.changed`、`settled` → `job.done`（`output` 事件忽略）；
    旧代（≤0.1.6）`onJobsChanged` / `onJobDone` 两个监听器。caller 类型同批换代（旧代 live Agent / 新代 `SessionId`）；
...
- harness 版本随壳绑定（当前值即 `package.json` 的 `dshRuntime.dsh`；**2026-09-25 起自动巡检暂停**，见 D51——随包版本暂时钉在最后一个验证可用的 `0.1.7-alpha.2`）；打包时由 setup-runtime.mjs 安装（`DSH_RUNTIME_DSH_VERSION` 可覆盖）。
...
- **macOS 多架构自动更新（只在签名分支成立）**：arm64/x64 各自平台生成运行时，CI 分两个 job 各产 dmg，**签名分支额外产 zip**。原因：**electron-updater 在 macOS 上只认 zip**——`MacUpdater` 用 `findFile(files,'zip',['pkg','dmg'])` 挑包（dmg 被显式排除），清单里没有 zip 时 `downloadUpdate()` 必抛 `ERR_UPDATER_ZIP_FILE_NOT_FOUND`。因此未签名构建**不产出 zip**（未签名 zip 会被 Squirrel.Mac 拒绝，发了也装不上），壳据清单缺 zip 诚实提示「请手动下载安装」（`src/main/updatePayload.ts`，D43）；签名分支用 `npx electron-builder --mac dmg zip` 打包，再由 `scripts/package-macos.mjs --zip` 公证+钉票 .app 后**用钉票后的 .app 重打 zip**（钉票改字节，打包期那份 zip 与其 blockmap 都已作废）。独立的 `merge-mac-manifest` job（`scripts/merge-mac-manifest.mjs`）把两个架构的 zip+dmg 合并成一份 `latest-mac.yml`，sha512/size **一律由实际文件现算**。
- 更新安装：Windows NSIS 用 `quitAndInstall(true, true)`——**静默安装 + 强制重启**（应用内对话框已经确认过一次，再弹一层 NSIS 向导是重复确认；`isForceRunAfter` 若不传，只是 Finish 页的一个勾选框，与对话框承诺的"安装完成后自动重启"不符）。下载由壳显式 `downloadUpdate()` 发起（`autoDownload=false`）：自动下载发生在 electron-updater 内部、壳拦不住，而"这版装不了"的判断必须在下载之前做完（D43）。Windows 的差分更新依赖 `latest.yml` 之外的 `<exe>.blockmap`，漏传只会退回整包下载（多下 ~160 MB），不会失败。
...
| D44 | **profile「目录名」与「身份名」分离：目录仍是自有 `dsh-workbench`，交给启动器的身份名改成官方的 `desktop`**。官方 `runProfile({profile})` 只把 `profile` 当**身份**（`profileContext.name`），profile **目录**来自我们显式传入的 `resolvedProfile`；而官方组合树里有两处 desktop-only 行按这个身份开关：`dsh-base` 的 `deepseek-account.desktopPlatform`（决定账号请求是否带 `x-client-platform: desktop-win`）与 `dsh-web-app` 的 `ui-sidebar-browser`。本壳长期报 `dsh-workbench` → 官方账号插件认为"不是桌面端" → 左下角登录拿不到桌面身份，平台在 `auth_exchange` 阶段拒绝（真机日志 `sign-in failed { errorCode: 'network' }`，而 `fetch` 层已被同一时刻成功的 `auth_init` 排除）；平台拒绝了也报 `network`，所以从日志分类上看不出是"被拒"还是"断网"——这正是它难查的原因 | 目录名必须保留自有（官方 CLI 硬拒绝 `--profile desktop`，那是官方 Electron 应用的保留名），身份名照官方报 `desktop`；两件事分开后盘上不占官方保留目录，组合树里仍是桌面端（`packages/host/src/index.ts` 的 `DESKTOP_PROFILE_IDENTITY`） |
| D45 | **插件残留的根因与壳侧收敛**。官方插件管理器的事务失败语义是「还原 `package.json` + `pnpm-lock.yaml`」，**`node_modules` 不回滚**；再叠上跨壳版本换 pnpm 主版本（store 布局换代 → `ERR_PNPM_UNEXPECTED_STORE`，每次包操作必失败），真机上留下三类不一致：① 声明了依赖、包不在盘上（真机 `dsh-better-sidebar`；harness 报 `cannot resolve profile bundle …`，0.1.7 前直接 fatal → 连续三次把应用推进安全模式）；② 包在盘上、清单里没有（真机 `dsh-commandcode-goat` 由 pnpm 托管、`dsh-opencode-go` 是手链接进 `E:\Dsh\Plugins` 的）；③ 锁文件与清单脱节 | 修复分两层：**bundle 对账加解析检查**（`isBundleResolvable`：profile `node_modules` 或随包 dsh 树里有 `package.json` 才算数，否则移出 `dsh.profile.bundles`——依赖与已装包一律不动，插件在官方「插件」页里仍可见、可重装）；**profile 依赖体检 + 修复**（`profileDeps.ts` 纯判定、`profileRepair.ts` 执行）：缺包/锁文件脱节 → 随包 pnpm 跑一次 `pnpm install`；「pnpm 不认、清单也没声明」的 dsh 插件目录 → 删除（**链接只删链接本身**，绝不顺着链接删被指向的目录）。离线失败不阻塞启动，按状态指纹 6 小时内不重跑，日志/通知说清是哪一档不一致 |
| D46 | **「运行时文件缺失」必须与「插件崩溃」分开**。真机（2026-09-23，0.8.5）：卡巴斯基主动防御把随包 `dsh-desktop-host/lib/index.js` 判成 `PDM:Trojan.Win32.Generic` 并清除（未签名 Electron + 内嵌 Node 运行时的行为启发式误报），壳却按"harness 崩溃"连推三次 → 安全模式，用户看到的结论是"插件坏了" | 两层前置检查把这类故障说清楚：① `resolveRuntime()` 校验关键随包文件（Node / pnpm / Host 入口 / bridge / harness 包 / Web 前端 dist），缺了直接报「哪个文件缺 + 很可能被安全软件隔离 + 加白名单后重装」；② `HostManager.spawn()` 再查一次 Host 入口，缺失时**不重启、不计入安全模式**，走 `onRuntimeDamaged` 弹可执行说明（`test/host-runtime-damaged.test.mjs` 锁契约：不 spawn、不排重启、状态 `stopped`）。配套 `docs/ANTIVIRUS-FALSE-POSITIVE.md`（哈希自查 + 加白名单 + 报误报路径 + 长期解法=EV 签名）。**后续（D50）**：Host 入口改为随壳打进 app.asar，这一层只剩"连壳自带的都不在"的真损坏 |
| D47 | **不再随包 `node.exe`：Host 与 pnpm 都由应用自己的 Electron 二进制以 Node 模式运行**（官方桌面端同形）。杀软行为启发式（PDM）对「未签名解释器镜像从深层目录被执行 → 再拉子进程」这类形状最敏感，真机被隔离的正是随包 Node 跑起来的那条链；同时 `--expose-internals`（pnpm 启动参数）在行为引擎眼里同样显眼 | ① `resolveRuntime().node = process.execPath` + `nodeRunAsNode: true`，`hostProcess.ts` 以 `ELECTRON_RUN_AS_NODE=1` spawn（该变量随环境传给 Hook 的所有子进程，插件里的 `process.execPath` 也保持 Node 语义）；② `packages/host` 的 `packageManager` 与壳的 profile 修复同用一个二进制，并**去掉 `--expose-internals`**（实测 pnpm 11 的 install/add/remove 都不需要）；③ 因此必须把 electron **精确**钉在 `44.0.0`——harness 的 `node-addon-require-builtin` 只认 Electron `43.0.0 / 44.0.0 / 45.0.0-alpha.6` 的运行时指纹（真机实测 43.4.0 会以 `Unsupported/no-context` 拒绝启动，native addon 里那张表就是证据），`setup-runtime` 把 `electronVersion` + Electron 内置的 `nodeVersion` 写进 `desktop-runtime.json`；④ 安装包 `extraResources` 过滤 `!node/**`（构建期那份便携 Node 只当工具链）：少一个未签名解释器镜像、少约 50 MB / 约 500 个文件；⑤ 顺带按目标架构剔除 node-pty 的 ConPTY 二进制（真机 x64 包里那两份 win10-arm64 正是被杀软一起清掉的、且永远不会被执行） | 
| D48 | **profile 依赖「体检自动、修复手动」**：0.8.5 那版在启动路径上直接跑 `pnpm install`——那是"应用自己装软件"的形状（行为启发式最敏感），用户也无从预期机器上发生了什么 | 启动期只做**只读体检**（`inspectProfile`：清单 / 已装 / `pnpm-lock.yaml` / `.modules.yaml` 四方对账），结论进托盘；发现漂移时托盘多一条「修复插件环境（缺失 N · 残留 M）…」，点了才弹确认并执行（`explicit: true` 忽略"同状态静默期"），修完重启 Host。`DSH_DESKTOP_PROFILE_REPAIR=force` 保留给开发/CI 自动跑 |
| D49 | **Host/pnpm 用操作系统证书库；登录页开在应用内窗口**。真机（2026-09-23）：左下角登录一直"登录失败"，日志是 `auth_init … errorCode: 'network'`。用 `tls.connect` 看链才发现：`*.deepseek.com` 的证书链被**卡巴斯基「加密连接扫描」**用自己的自签根（`Kaspersky Anti-Virus Personal Root Certificate`，已装进 Windows 根证书库）重建，Chromium/浏览器信它、**Node 只认自己打包的 CA 列表** → Host 进程里所有 HTTPS 都 `SELF_SIGNED_CERT_IN_CHAIN`（登录、模型请求、余额查询、pnpm 拉包一起挂；而应用内网页与 electron-updater 正常，正好是"网页能上、应用连不上"的典型症状） | ① 宿主 spawn 与 pnpm 调用都加 **`--use-system-ca`**（Node 24 官方开关，实测同机同请求从 `failed/network` 变成 `waiting-browser` 并拿到 `https://platform.deepseek.com/dsh/authorize`）；② 壳自己的 API Key 自检改走 **Chromium 网络栈**（Electron `net.fetch`）；③ 登录页默认开**应用内窗口**（独立 `persist:dsh-login` 分区、sandbox、无 preload），导航白名单只放行平台授权域与本机回环回调（`/oauth/callback`），加载/建窗失败自动退回系统浏览器，`DSH_DESKTOP_LOGIN_BROWSER=1` 可强制浏览器；成功/失败/超时都自动关窗（成功再通知一次）。理由：登录**必须**依赖用户浏览器完成平台授权，但"系统默认浏览器没配好/被劫持/窗口被切走"都会让用户以为功能坏了——内置窗口去掉这个外部依赖，同时保留浏览器兜底 |
| D50 | **Host 入口不再当"运行时树里的散件"：随壳打进 `app.asar`，树里那份只作兜底**。真机（2026-09-25，0.8.8）补齐了 D46 的另一半证据：应用**没有运行**的某段时间里（9-23 05:51 最后一次正常 ready → 9-25 13:29 起连续两次启动失败），卡巴斯基把 `resources\dsh\node_modules\dsh-desktop-host\lib\index.js`（7711 B，sha256 `8ece84a6…`）删了。按 `desktop-runtime.json` 的 10883 条清单逐文件对账：**只缺这一个**、其余大小/哈希全对，且仓库与 `release/win-unpacked` 里那份字节一致——不是安装不完整，也不是文件被改写，而是"针对这个文件的判定"。它是壳自身代码（`packages/host/src/index.ts`，MIT 移植）的构建产物，却正好落在启发式最敏感的形状上（安装目录深层里一个未签名的脚本 + 会拉起子进程），而它又是启动硬前提，于是 D46 的检查只能把它转成一句"请加白名单后重装" | ① `scripts/build.mjs` 把**同一份源码**打进 `dist/main/host-entry.cjs`（随 `files: dist/**` 进 app.asar；banner 把 `<runtimeDir>/node_modules` 加进本模块搜索路径，`@deepseek-ai/*` 仍由运行时树提供——"壳 + dsh + pnpm 是一个签名更新单元"没被拆开；`define: {'import.meta.main': true}` 让 CJS 下照旧直接执行 main）；② 新增 `src/main/hostEntry.ts` 做入口决策：壳自带副本优先 → 运行时树副本兜底 → 两处都没有才算损坏；`resolveRuntime()`、`HostManager.spawn()`、`DesktopHostProcess` 全部改用它，于是**树里那份被杀软清掉不再挡住启动**；③ 契约测试 `test/host-entry.test.mjs`（决策顺序）与 `test/host-runtime-damaged.test.mjs`（树入口缺失但壳自带副本在场 → 照常 spawn、不报损坏）；④ 实测：从 asar 里加载该入口能完整引导真实 harness 并报 ready（`dsh 0.1.7-alpha.2`，隔离 `DSH_HOME`、端口 19499，Electron 44 以 `ELECTRON_RUN_AS_NODE=1` 运行）；`scripts/e2e-bridge.mjs` 也改为优先用壳自带入口，让这条路径进 CI；⑤ 仍然保留的代价：树里那份散件还在（随包运行时是完整闭包），杀软仍可能隔离它并弹窗——但那时应用照常工作（要彻底不弹窗，得在下次重建运行时树时用 `scripts/runtime-file-policy.mjs` 把它从 `resources/dsh` 里剔除） |
| D51 | **暂缓跟随官方 0.1.7-rc.1+：桌面客户端开始要求"官方私有桌面壳的原生桥"**。真机（2026-09-25，壳 v0.8.11 = dsh 0.1.7-rc.2）：应用能启动、Host 也 `host ready`，但**工作台打不开**——`workspace boot failed: web boot: 24 entries did not activate`，唯一失败项是 `@deepseek-ai/dsh-client-shortcuts: failed`，其余 23 项 pending 等它提供的 `shortcuts` 服务。根因在 rc.2 的客户端 bundle：`detectEnvironment()` 看见 `document.documentElement.dataset.platform` 就判定 `runtime === 'desktop'`，随后**强制要求** `window.dshDesktop.keyboard`，缺失即 `throw new Error("Desktop keyboard bridge unavailable")`。对照组：同一个 host、同一份 boot manifest（65 条 entry、同一 rev）在**官方 web 路径**下用浏览器实测正常（不进入该分支，内测声明页正常渲染）——所以不是 harness 坏了，而是"桌面壳必须提供的能力面"变了，而那是官方**未发布**的私有桌面壳（D26）才有的东西 | ① 随包 harness 回到**最后一个验证可用**的 `0.1.7-alpha.2`，壳版本升到 v0.8.12（`package.json.dshRuntime.dsh` + `packages/host` 跟随依赖一起回退），保证用户端立刻可用；② `.github/workflows/upstream-dsh.yml` **停用 schedule + 增加"暂停守卫"步骤**：即使手动触发也会拒绝同步新版本、写明原因并让 run 失败（避免无人值守再自动发一版打不开的包）；③ 恢复自动巡检前要补的桥接面（rc.2 客户端实际读取的 API）：preload 暴露 `window.dshDesktop.keyboard` = `{ subscribe(handler) → dispose, closeWindow(revision) }`（把桌面原生输入——菜单/快捷键——喂给客户端的命令注册表，事件形如 `{ revision, kind: 'menu' \| 'keyboard' \| 'webview', … }`），`window.dshDesktop.shortcuts` = `{ get(definitions), edit(edit, revision), subscribe(publish), recording(active), dispose() }`（快捷键配置的桌面持久化适配器，替代 web 的 localStorage 实现），以及 `document.documentElement.dataset.platform`（正是它把运行时判成 desktop）；④ 等上游稳定（rc 线不再动、或 stable 版把这套桥的契约写进公开文档）再实现并恢复自动化 |
| D31 | **签名按官方流程**：Windows EV/SafeNet 令牌签名（`scripts/windows-sign.mjs`，环境不备则显式跳过、配一半则硬报错）；macOS Developer ID + notarytool + stapling（`scripts/package-macos.mjs`，`resources/dsh`、`resources/runtime` 排除签名） | 官方 build pipeline 的等价物；细节与所需环境变量见 docs/SIGNING.md |
