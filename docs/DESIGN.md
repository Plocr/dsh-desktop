...
- **上下文数值与构成直接展示**：`~X / Y` 明细与构成只在 harness 展开上下文明细后渲染于 DOM（且**收起后 DOM 被移除**，实测）——面板在明细缺失时**自动展开一次读取并缓存**（读取后自动收起，不干扰用户手动展开；防御超时 10s），此后构成**直接展示**（系统/工具/对话 chips）；缓存 60s 或百分比变化 ≥2 时自动刷新；未获取到前按 **百分比 × 窗口（deepseek-v4 系列 1M，`CONTEXT_WINDOW_DEFAULT`）估算**并标注 ≈。
