 ...
 * `ctx.connection.rpc.call('/rpc', method, payload)` 调用。
 *
 * 配置（loader 条目 config，见 README「余额/用量」与「离线 token 统计」两节）：
 * ```yaml
 * config:
 *   balance:
 *     providers:
 ...
 *         path: /credits
 *   tokenizer:
 *     dir: E:\Dsh\deepseek_v3_tokenizer\deepseek_v3_tokenizer
 *     script: E:\Dsh\ui-dashboard-repo\scripts\tokenize.py
 * ```
 */
 ...
 * 脚本默认值解析为**包内** scripts/deepseek_tokenize.py（install.mjs 安装时会一并复制），
 * 避免写死某台机器的绝对路径；解析失败再回退历史默认值。
 */
function tokenizerPaths(config) {
  let script = 'E:\\Dsh\\ui-dashboard-repo\\scripts\\deepseek_tokenize.py'
  try {
    const here = fileURLToPath(new URL('.', import.meta.url)) // <pkg>/lib/
    const bundled = path.join(here, '..', 'scripts', 'deepseek_tokenize.py')
    if (existsSync(bundled)) script = bundled
  } catch {
    /* 非文件加载（打包器内联等）时回退默认值 */
  }
  return {
    dir: config?.tokenizer?.dir ?? 'E:\\Dsh\\deepseek_v3_tokenizer\\deepseek_v3_tokenizer',
    script: config?.tokenizer?.script ?? script
  }
}
...
async function tokenizeTexts(ctx, texts, config) {
  ...
  const handle = sub.spawn({
    argv: ['python', paths.script, paths.dir],
    cwd: paths.dir,
    stdio: {
...
async function offlineMessageStats(ctx, sessionId, config) {
  ...
  const [userCounts, assistantCounts] = await Promise.all([
    tokenizeTexts(ctx, users, config),
    tokenizeTexts(ctx, assistants, config)
  ])
...
export function apply(ctx, config) {
  ctx.effect(() => ctx.connection.rpc.handle('/rpc', async (endpoint, payload) => {
    ...
    if (endpoint === 'dir') {
      ...
      if (path === '') return { ok: true, value: { status: 'error', message: 'path required' } }
      const fs = ctx.get('fs')
      if (fs === undefined || typeof fs.listDir !== 'function') return { ok: true, value: { status: 'error', message: 'fs service unavailable' } }
      try {
        ...
        return { ok: true, value: { status: 'error', message: e instanceof Error ? e.message : String(e) } }
      }
    }
    if (endpoint === 'tokenize') {
      const texts = payload !== null && typeof payload === 'object' && Array.isArray(payload.texts) ? payload.texts : []
      // 上限防御：直接 RPC 无 offline 路径的 400 条截断，超大批量会撑爆 subprocess stdout 缓冲
      if (texts.length > 500) {
        return { ok: true, value: { status: 'error', message: `too many texts (${texts.length} > 500)` } }
      }
      try {
        return { ok: true, value: { status: 'ok', counts: await tokenizeTexts(ctx, texts, config) } }
      } catch (e) {
        return { ok: true, value: { status: 'error', message: e instanceof Error ? e.message : String(e) } }
      }
    }
    if (endpoint === 'offline-messages') {
      const sessionId = payload !== null && typeof payload === 'object' && typeof payload.sessionId === 'string' ? payload.sessionId : ''
      if (sessionId === '') return { ok: true, value: { status: 'error', message: 'sessionId required' } }
      try {
        return { ok: true, value: await offlineMessageStats(ctx, sessionId, config) }
      } catch (e) {
        return { ok: true, value: { status: 'error', message: e instanceof Error ? e.message : String(e) } }
      }
    }
  ...
  }, { authority: 'loopback' }), 'ui-dashboard: current-model/balance/dir/tokenize/offline-messages rpc')
}

// 供测试与跨版本回归直接调用（cordis 只关心 name/apply）
export { readStoredEvents }
